...
impl<S: ToScalar> Mul<S> for Expression<Building> {
    type Output = Expression<Building>;
    fn mul(self, rhs: S) -> Self::Output {
        let mul = rhs.to_scalar_from_host(self.sym_ref());

        let (sums, terms, constant) = get_sums_terms_and_constant(self);

        let new_constant = constant.map(|x| x * mul);
        let new_terms = terms.iter().map(|x| x * mul);
        let new_sums = sums.iter().map(|x| x * mul);
